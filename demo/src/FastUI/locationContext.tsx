...
export function LocationProvider({children}: {children: ReactNode}) {
  ...
  const value: LocationState = {
    ...pureLocation,
    fullPath,
    goto: useCallback((pushPath: string, clear?: boolean) => {
    ...
    setQuery: useCallback((query: Record<string, string> | URLSearchParams) => {
      const newQuery = new URLSearchParams(query)
      pushState(pureLocation.path, newQuery, pureLocation.hash)
    }, [pushState, pureLocation]),
    setQueryArg: useCallback((name: string, value: string | false) => {
      if (value === false) {
